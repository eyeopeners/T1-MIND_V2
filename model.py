...
class TriLevelPriorFusionNetwork(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.text_encoder = TextPriorEncoder(
            ...
            dim_out=256,
            use_local_clip=cfg.get("use_local_clip", True),  # 从配置读取
            local_model_path=cfg.get("local_clip_path", "./pretrained_models/clip-vit-base-patch32")
        )
    ...
    def forward(self, batch):
        ...
        Args:
            batch: dict with keys:
                ...
                - text_class: [B]
                - cover_ctx: [B, S, 360] 或 [S, 360]
                - cover_sub: [B, S, 66] 或 [S, 66]
        """
        B = batch['slices'].shape[0]
        
        # ========== Level 1: 文本先验 ==========
        text_class = batch['text_class']  # [B]
        text_prior = self.text_encoder(text_class)  # [B, 256]
        
        # ========== Level 2: 切片选择 + ROI提取 ==========
        slice_out = self.slice_selector(
            batch['slices'],  # [B, S, 1, H, W]
            text_prior        # [B, 256]
        )
        
        selected_features = slice_out['features']  # [B, K, 256]
        selected_indices = slice_out['selected_indices']  # [B, K]
        attention_scores = slice_out['attention_scores']  # [B, S]
        
        # 获取覆盖矩阵（确保是[B, S, R]格式）
        cover_ctx = batch['cover_ctx']  # [B, S, 360] 或 [S, 360]
        cover_sub = batch['cover_sub']  # [B, S, 66] 或 [S, 66]
        
        # ========== 统一维度处理 ==========
        if cover_ctx.dim() == 2:
            # [S, 360] -> [B, S, 360]
            cover_ctx = cover_ctx.unsqueeze(0).expand(B, -1, -1)
        if cover_sub.dim() == 2:
            # [S, 66] -> [B, S, 66]
            cover_sub = cover_sub.unsqueeze(0).expand(B, -1, -1)
        # ==================================
        
        # 提取皮层和皮层下ROI特征
        roi_features_ctx = self._extract_roi_from_selected(
            selected_features, selected_indices, cover_ctx
        )  # [B, 360, 256]
        
        roi_features_sub = self._extract_roi_from_selected(
            selected_features, selected_indices, cover_sub
        )  # [B, 66, 256]
        
        # ========== Level 3: 图网络编码 ==========
        mind_out = self.mind_encoder(
        batch['mind'],        # [B, 360, 360] - mind_matrix
        roi_features_ctx,     # [B, 360, 256] - roi_features_img
        self.group_prior      # [360, 360] - group_prior（组间先验）
        )
        
        # 提取图级嵌入和节点特征
        z_ctx = mind_out['node_features']  # [B, 360, 256]
        graph_embedding = mind_out['graph_embedding']  # [B, 256]
        
        # 皮层下编码
        z_sub = self.subcortex_encoder(
            roi_features_sub  # [B, 66, 256]
        )  # [B, 64]
        
        # ========== Level 4: 个体先验 ==========
        individual_prior = self.individual_prior_encoder(
            batch['mind'],         # [B, 360, 360] - MIND矩阵
            batch['demographics']  # [B, D_demo] - 人口学信息
        )  # [B, 128]
        
        # ========== 跨模态融合 ==========
        fusion_out = self.cross_modal_fusion(
        z_img=text_prior,            # [B, 256] - 文本先验
        z_graph=graph_embedding,     # [B, 256] - 图级嵌入
        z_subcortex=z_sub,           # [B, 64] - 皮层下特征
        z_individual=individual_prior  # [B, 128] - 个体先验
        )
        
        fused = fusion_out['fused_features']  # [B, 512]
        ...
        # 返回
        return {
        'logits_3way': preds['logits_3way'],  # [B, 3]
        'logits_nc_ad': preds['logits_nc_ad'],  # [B, 2]
        'logits_nc_mci': preds['logits_nc_mci'],  # [B, 2]
        'logits_mci_ad': preds['logits_mci_ad'],  # [B, 2]
        'logits_ad_vs_non': preds['logits_ad_vs_non'],  # [B, 2]
        'logits_mci_vs_non': preds['logits_mci_vs_non'],  # [B, 2]
        'text_prior': text_prior,  # [B, 256]
        'individual_prior': individual_prior,  # [B, 128]
        'z_graph': z_ctx,  # [B, 360, 256] - 节点特征
        'graph_embedding': graph_embedding,  # [B, 256] - 图级嵌入
        'selected_features': selected_features,  # [B, K, 256]
        'selected_indices': selected_indices,  # [B, K]
        'attention_scores': attention_scores,  # [B, S]
        'modal_weights': modal_weights,  # [B, 4]
        'node_attention': mind_out['node_attention']  # [B, 360]
        }
    
    def _extract_roi_from_selected(self, selected_features, selected_indices, cover_matrix):
        """
        从选中的切片中提取ROI特征
        
        Args:
            selected_features: [B, K, D] - 选中切片的特征
            selected_indices: [B, K] - 选中切片的索引
            cover_matrix: [B, S, R] 或 [S, R] - 覆盖矩阵
            
        Returns:
            roi_features: [B, R, D] - ROI特征
        """
        B, K, D = selected_features.shape
        
        # ========== 处理cover_matrix的维度 ==========
        if cover_matrix.dim() == 2:
            # [S, R] -> [B, S, R]
            S, R = cover_matrix.shape
            cover_matrix = cover_matrix.unsqueeze(0).expand(B, -1, -1)
        elif cover_matrix.dim() == 3:
            # [B, S, R]
            B_cover, S, R = cover_matrix.shape
            assert B_cover == B, f"Batch size mismatch: {B_cover} != {B}"
        else:
            raise ValueError(f"Invalid cover_matrix dim: {cover_matrix.dim()}")
        # ===========================================
        ...
        batch_idx = torch.arange(B, device=selected_features.device).view(B, 1, 1).expand(B, K, R)
        slice_idx = selected_indices.unsqueeze(-1).expand(B, K, R)  # [B, K, R]
        roi_idx = torch.arange(R, device=selected_features.device).view(1, 1, R).expand(B, K, R)
        
        # 索引: cover_matrix[batch_idx, slice_idx, roi_idx]
        selected_cover = cover_matrix[batch_idx, slice_idx, roi_idx]  # [B, K, R]
        
        # 加权聚合：每个ROI由覆盖它的切片特征加权求和
        # selected_features: [B, K, D]
        # selected_cover: [B, K, R]
        # 结果: [B, R, D]
        
        roi_features = torch.einsum('bkd,bkr->brd', selected_features, selected_cover)  # [B, R, D]
        
        # 归一化（按覆盖权重之和）
        cover_sum = selected_cover.sum(dim=1, keepdim=True).transpose(1, 2)  # [B, R, 1]
        roi_features = roi_features / (cover_sum + 1e-6)
    ...
    def compute_loss(self, out, labels, cfg):
        ...
        if mask_nc_ad.sum() > 0:
            ...
            logits_nc_ad = out['logits_nc_ad'][mask_nc_ad]
        ...
        if mask_nc_mci.sum() > 0:
            ...
            logits_nc_mci = out['logits_nc_mci'][mask_nc_mci]
        ...
        if mask_mci_ad.sum() > 0:
            ...
            logits_mci_ad = out['logits_mci_ad'][mask_mci_ad]
        ...
        loss_ad_vs_non = F.cross_entropy(out['logits_ad_vs_non'], labels_ad)
        
        # 6. MCI vs 非MCI (所有样本)
        labels_mci = (labels == 1).long()
        loss_mci_vs_non = F.cross_entropy(out['logits_mci_vs_non'], labels_mci)
        ...
        loss_total = (
            precision[0] * loss_3way + self.log_vars[0] +
            precision[1] * loss_nc_ad + self.log_vars[1] +
            precision[2] * loss_nc_mci + self.log_vars[2] +
            precision[3] * loss_mci_ad + self.log_vars[3] +
            precision[4] * loss_ad_vs_non + self.log_vars[4] +
            precision[5] * loss_mci_vs_non + self.log_vars[5] +
            precision[6] * loss_text + self.log_vars[6] +
            precision[7] * loss_sparse + self.log_vars[7]
        )
        
        return {
            'loss_total': loss_total,
            'loss_fused': loss_3way,  # 保持与train.py兼容
            'loss_ad': loss_ad_vs_non,
            'loss_nm': loss_nc_mci,
            'loss_text': loss_text,
...
class TextPriorEncoder(nn.Module):
    ...
    def __init__(self, clip_model_name="openai/clip-vit-base-patch32", 
                 dim_out=256, 
                 use_local_clip=True,
                 local_model_path="./pretrained_models/clip-vit-base-patch32"):
        super().__init__()
        
        # 加载CLIP模型
        if use_local_clip:
            # 使用本地模型
            print(f"Loading CLIP from local: {local_model_path}")
            try:
                ...
                print("✓ CLIP model loaded successfully from local")
            except Exception as e:
                print(f"⚠️ Failed to load local CLIP model: {e}")
                print(f"   Falling back to online loading...")
                self.clip_model = CLIPModel.from_pretrained(clip_model_name)
                self.clip_processor = CLIPProcessor.from_pretrained(clip_model_name)
        else:
            # 在线加载
            print(f"Loading CLIP from HuggingFace: {clip_model_name}")
            self.clip_model = CLIPModel.from_pretrained(clip_model_name)
            self.clip_processor = CLIPProcessor.from_pretrained(clip_model_name)
    ...
    def _precompute_text_embeddings(self):
        ...
        # CLIP编码
        inputs = self.clip_processor(  # 修复：使用self.clip_processor而不是self.processor
            text=text_list,
            return_tensors="pt",
            padding=True,
            truncation=True
...
# model.py - 修改TextGuidedSliceSelector (完整版本)
...
class TextGuidedSliceSelector(nn.Module):
    ...
    def forward(self, x, text_prior):
        """
        Args:
            x: [B, S, C, H, W] - 切片堆栈（C=1为单通道MRI）
            text_prior: [B, D_text] - 文本先验嵌入
            
        Returns:
            dict with features, attention_scores, selected_indices, topk_values
        """
        B, S, C, H, W = x.shape
        ...
        # Stage 4: 计算注意力分数
        data_attention_logits = self.attention_head(contextualized).squeeze(-1)
        combined_logits = data_attention_logits + 0.5 * torch.log(text_attention_prior + 1e-8)
        attention_scores = torch.softmax(combined_logits, dim=-1)
        ...
        selected_features = contextualized[batch_indices, topk_indices]
        
        # Stage 6: 压缩
        compressed_features = self.compress(selected_features)
